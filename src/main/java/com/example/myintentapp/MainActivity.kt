    ...
    override fun onClick(v: View) {
        when (v.id) {
            ...
            R.id.btn_dial_number -> {
                        val phoneNumber = "082140759842"
                        val dialIphoneIntent = Intent(Intent.ACTION_DIAL,)
                        Uri.parse("tel:$phoneNumber")
                        startActivity(dialIphoneIntent)}
            R.id.btn_move_activity_with_object -> {
                val person = Person("AFRIANSYAH", "afriansyah.wahyu565@gmail.com", "Malang")
                val moveWithObjectActivityActivity = Intent(
                    this@MainActivity,
                    MoveWithObjectActivity::class.java
                )
                moveWithObjectActivityActivity.putExtra(MoveWithObjectActivity.EXTRA_PERSON, person)
                startActivity(moveWithObjectActivityActivity)
            }
